...
const SearchProduct: React.FC<SearchProductProps> = () => {
  ...
  const productName = router.query.search;
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await axios.get(`http://localhost:3000/product/search/${productName}`);
  ...
  try {
    const res = await axios.get(
      `http://localhost:3000/product/search/${productName}`,
      // Add query parameters for sorting
      {
        params: {
          sortBy: 'price', // specify the field to sort by (adjust it based on your API)
          sortOrder: 'asc', // specify the sort order (asc for ascending)
        },
      }
    );
    ...
    try {
      const res = await axios.get(
        ...
        // Add query parameters for sorting
        {
          params: {
            sortBy: 'price', // specify the field to sort by (adjust it based on your API)
            sortOrder: 'desc', // specify the sort order (asc for ascending)
          },
        }
      );
    ...
    <Layout>
      <div className="bg-gray-100 min-h-screen">
        <div className="container mx-auto p-4 relative">
          ...
          {isDropdownOpen && (
            <div
              className="z-10 bg-white divide-y divide-gray-100 rounded-lg border-2 border-gray-400 shadow w-40 absolute top-16 right-4"
            >
